...
import { getApplication, updateApplication } from "@/models/application";
...
export async function GET(
  ...
  try {
    const result: API.ModelRes = await getApplication(params.id);
...
export async function PUT(
  ...
  try {
    ...
    const result: API.ModelRes = await updateApplication(
      params.id,
      await request.json(),
      session?.user?.email!
    );
