...
function JobList({
  ...
  const [data, setData] = useState<API.Job[]>([]);
  ...
  const [selectedItem, setselectedItem] = useState<API.Job | null>(null);
      ...
      .then(({ data }) => {
        ...
        if (data.meta.code === "OK") {
          ...
          api["warning"]({
            ...
            description:
              data.message ||
              "Failed to get jobs, please contact administrator",
          });
        }
      })
  ...
  const handleDelete = (ids: React.Key[]) => {
    confirm({
      title: "Are you sure to delete selected jobs",
      icon: <TriangleAlert color='#ff4d4f' className='mr-2' />,
      content: "Deleted jobs will not be able to recover",
      okType: "danger",
      onOk() {
        return axios
          .delete(url, {
          ...
          .then(({ data }) => {
            if (!data.code) {
              ...
              api["warning"]({
                ...
                description:
                  data.message ||
                  "Failed to delete jobs, please contact the administrator.",
              });
            }
          })
          .catch((error: API.AxiosError) => {
  ...
  const handleItemAction = ({ key }: { key: string }) => {
    ...
    switch (action) {
      case "view":
        message.warning("Job detail is in progress");
  ...
  const rowSelection: TableRowSelection<API.Job> = {
    selectedRowKeys,
                      ...
                      items: [
                        {
                          label: (
                            <div className='flex items-center justify-between'>
                              ...
                              <Tooltip title={"Create new job by manual"}>
                                <Info size={16} />
                              </Tooltip>
                            </div>
                          ),
                        ...
                        },
                        {
                          label: (
                            <div className='flex items-center justify-between'>
                              ...
                              <Tooltip
                                title={"Deleted jobs can not be restored"}
                              >
                                <Info size={16} />
                              </Tooltip>
                            </div>
